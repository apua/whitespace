...
def run_wvm(whitespace_code, lexical_token='STL'):
    ...
    def compile_to_IR(
        ...
        IR = [ 'lambda stack,heap,LR,PC,putchar,getchar:'+T[k][1].format(to_num(v) if k<3 else L.get(v)) for k,v in IR ]
    ...
    def interprete_IR(text, stack=[], heap={}, LR=[], PC=[0]):
        ...
        while PC[0] is not None:
            ...
            eval(instruction)(stack,heap,LR,PC,putchar,getchar) # do something, then PC+=1 or jump
